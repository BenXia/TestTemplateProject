...
- (UIWindow *)window {
    if (!_window) {
        ...
        BaseWindowRootVC *rootViewController = [BaseWindowRootVC createRootViewControllerWithStatusBarStyle:[UIApplication sharedApplication].statusBarStyle];
...
- (UIView *)backgroundView {
    if (!_backgroundView) {
        ...
        _backgroundView.backgroundColor = RGBA(0, 0, 0, 0.5);
...
- (void)addBottomPopoverVC:(BasePopoverVC *)bottomPopoverVC {
    @synchronized (g_bottomPopupVCArray) {
        [g_bottomPopupVCArray addObject:bottomPopoverVC];
    }
}

- (void)removeBottomPopoverVC:(BasePopoverVC *)bottomPopoverVC {
    @synchronized (g_bottomPopupVCArray) {
